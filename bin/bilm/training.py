...
def train_curriculum(options, data, n_gpus, tf_save_dir, tf_log_dir, initial_competence, competence_increment,
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        data_gen = data.iter_batches(batch_size * n_gpus, unroll_steps, competence, competence_increment)
        for batch_no, batch in enumerate(data_gen, start=1):
            batch_no_count = batch_no
            # slice the input in the batch for the feed_dict
            X = batch
...
def get_sentence_perplexity(options, ckpt_file, data, batch_size=1):
    '''
    Get the perplexity on each sample
    '''
    idx = 0
    bidirectional = options.get('bidirectional', False)
    char_inputs = 'char_cnn' in options
